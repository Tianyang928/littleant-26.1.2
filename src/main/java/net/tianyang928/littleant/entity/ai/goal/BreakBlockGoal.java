...
public class BreakBlockGoal extends Goal {
    ...
    public void tick() {
        ...
        if (this.breakTime >= this.getBlockBreakTime()) {
            this.mob.level().destroyBlock(this.blockPos, true, this.mob, 512);
            clearTarget();
        }
    }

    private boolean isBlockReachable() {
        return this.mob.distanceToSqr(this.blockPos.getX() + 0.5D, this.blockPos.getY() + 0.5D, this.blockPos.getZ() + 0.5D) <= 4.0D * 4.0D;
    }
}
